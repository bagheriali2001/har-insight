<template>
    <div class="relative w-full h-full overflow-x-auto justify-center items-center">
            <div class="line pl-5" :style="[{'border-top': '2px solid green'}, {'width': `${items.filter(item => item.done).length * elemWidth}px`}]"></div>
            <div class="line pr-5" :style="[{'border-top': '2px solid gray'}, {'width': `${items.filter(item => !item.done).length * elemWidth}px`}, {'left': `${items.filter(item => item.done).length * elemWidth}px`}]"></div>
