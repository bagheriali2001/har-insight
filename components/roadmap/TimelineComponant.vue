<template>
	<div class="relative w-full h-full overflow-x-auto justify-center items-center z-0">
		...
		/>
		<div
			class="line pr-5 border-t-2 border-solid border-gray-300 mt-2"
			:style="[{'width': `${items.filter(item => !item.done).length * elemWidth}px`}, {'left': `${items.filter(item => item.done).length * elemWidth}px`}]"
		/>

		<div
			class="flex flex-row justify-center top-[35px] absolute w-100 h-100"
			:style="[{ left: `${elemWidth/2}px`}]"
		>
			<div
			...
			>
				...
				<p class="text-gray text-sm mb-1.5">
					{{ item.time || "NA" }}
				</p>
				<div
					class="relative flex items-center justify-center rounded-[50%] w-[30px] h-[30px] mx-auto item-center mt-2"
					:class="{ 'bg-emerald-500 shadow-zero': item.done, 'bg-gray-300': !item.done }"
				>
					<img
						v-if="item.done"
						:src="tickIcon"
...
const { items, elemWidth } = defineProps({
	items: Array, 
	elemWidth: Number
});
</script>
